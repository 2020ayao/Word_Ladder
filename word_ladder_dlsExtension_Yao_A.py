...
def DLS(current_state, goal_state, lim, explored, dict):
    if current_state == goal_state:
        ...
        for e in dict[current_state]:
            if e not in explored:
                ...
                result = DLS(e, goal_state, lim - 1, explored, dict)
                if result != None:
                    return result
                else:
                    explored.pop(e)
...
def iterative_deepening_search(start, goal, dict):
    countDepth = 0
    while True:
        solved = DLS(start, goal, countDepth, {start:"s"}, dict)
        if solved == "Solved!":
            return
        countDepth+=1
        print(countDepth)
...
def main():
   ...
   iterative_deepening_search(starting, goal, dict)
